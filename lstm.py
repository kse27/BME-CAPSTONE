...
Original file is located at
    https://colab.research.google.com/drive/1elJVebfX2HuvdwZJ3XRRM6WKixURklw8

updrs1:
'C(UniMod_4)LAPLEGAR','FIYGGC(UniMod_4)GGNR','P30086','VRGGEGTGYFVDFSVR','Q92520'

updrs2:
'P07998','LVNEVTEFAK','LSKELQAAQAR','FIYGGC(UniMod_4)GGNR','P19827'

updrs3:
'QHVVYGPWNLPQSSYSHLTR','LLEVPEGR','NLREGTC(UniMod_4)PEAPTDEC(UniMod_4)KPVK','TPC(UniMod_4)TVSC(UniMod_4)NIPVVSGKEC(UniMod_4)EEIIR','P13521'

updrs4:
'HGNVAEGETKPDPDVTER','VNHVTLSQPK','SILENLR','AQC(UniMod_4)GGGLLGVR','EDC(UniMod_4)NELPPRR'
"""
...
selected_columns = ['C(UniMod_4)LAPLEGAR', 'FIYGGC(UniMod_4)GGNR', 'P30086', 'VRGGEGTGYFVDFSVR', 'Q92520', 'updrs_1']
...
for patient_id in train_patients:
    patient_data = train_data[train_data['patient_id'] == patient_id][['C(UniMod_4)LAPLEGAR','FIYGGC(UniMod_4)GGNR','P30086','VRGGEGTGYFVDFSVR','Q92520','updrs_1']].values
    train_patient_sequences.append(patient_data)
print(train_patient_sequences)

for patient_id in test_patients:
    patient_data = test_data[test_data['patient_id'] == patient_id][['C(UniMod_4)LAPLEGAR','FIYGGC(UniMod_4)GGNR','P30086','VRGGEGTGYFVDFSVR','Q92520','updrs_1']].values
...
model = Sequential()
model.add(LSTM(units=50, input_shape=(None, 5), return_sequences=True))  # return_sequences=True를 추가하여 출력을 시퀀스로 반환합니다.
...
adam = Adam(lr=0.001)  # 원하는 학습률로 설정합니다.
...
# 모델 성능 평가 (예: 평균 제곱 오차)
y_true = [sequence[1:, -1] for sequence in test_patient_sequences]
mse = np.mean(np.square(np.concatenate(y_true) - np.concatenate(predictions)))
print("Mean Squared Error:", mse)

from keras.models import load_model

# 모델 저장
#model.save('/content/drive/MyDrive/BME/my_lstm_model.h5')

# 저장된 모델 불러오기
#loaded_model = load_model('my_lstm_model.h5')

loaded_model = load_model('/content/drive/MyDrive/BME/my_lstm_model.h5')
...
for patient_id in train_patients:
    patient_data = train_data[train_data['patient_id'] == patient_id][['P07998','LVNEVTEFAK','LSKELQAAQAR','FIYGGC(UniMod_4)GGNR','P19827','updrs_2']].values
    train_patient_sequences.append(patient_data)

for patient_id in test_patients:
    patient_data = test_data[test_data['patient_id'] == patient_id][['P07998','LVNEVTEFAK','LSKELQAAQAR','FIYGGC(UniMod_4)GGNR','P19827','updrs_2']].values
    test_patient_sequences.append(patient_data)
...
# 모델 저장
model.save('/content/drive/MyDrive/BME/my_lstm_model2.h5')
...
for patient_id in train_patients:
    patient_data = train_data[train_data['patient_id'] == patient_id][['QHVVYGPWNLPQSSYSHLTR','LLEVPEGR','NLREGTC(UniMod_4)PEAPTDEC(UniMod_4)KPVK','TPC(UniMod_4)TVSC(UniMod_4)NIPVVSGKEC(UniMod_4)EEIIR','P13521','updrs_3']].values
    train_patient_sequences.append(patient_data)

for patient_id in test_patients:
    patient_data = test_data[test_data['patient_id'] == patient_id][['QHVVYGPWNLPQSSYSHLTR','LLEVPEGR','NLREGTC(UniMod_4)PEAPTDEC(UniMod_4)KPVK','TPC(UniMod_4)TVSC(UniMod_4)NIPVVSGKEC(UniMod_4)EEIIR','P13521','updrs_3']].values
    test_patient_sequences.append(patient_data)
...
for patient_id in train_patients:
    patient_data = train_data[train_data['patient_id'] == patient_id][['HGNVAEGETKPDPDVTER','VNHVTLSQPK','SILENLR','AQC(UniMod_4)GGGLLGVR','EDC(UniMod_4)NELPPRR','updrs_4']].values
    train_patient_sequences.append(patient_data)

for patient_id in test_patients:
    patient_data = test_data[test_data['patient_id'] == patient_id][['HGNVAEGETKPDPDVTER','VNHVTLSQPK','SILENLR','AQC(UniMod_4)GGGLLGVR','EDC(UniMod_4)NELPPRR','updrs_4']].values
    test_patient_sequences.append(patient_data)
